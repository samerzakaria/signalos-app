...
def _build_agent_prompt(
    ...
    if dc:
        ...
        if tokens:
            ...
            # #8: the freed tokens must reach the agent too.
            lines.append(f"- Color scheme: {tokens.get('color_scheme', 'light')}")
            lines.append(f"- Border radius: {tokens.get('border_radius', '8px')}")
            lines.append(f"- Spacing unit: {tokens.get('spacing_unit', 8)}px")
            if tokens.get("type_scale"):
                lines.append(f"- Type scale: {tokens['type_scale']}")
        lines.append("")
...
def _render_foundation_file(
    ...
    if path == "src/product.css":
        # #8: the stylesheet honors the approved design tokens too.
        return _render_product_css(gen.get("design_constraints", {}) or {})
    return None
...
def _render_vitest_setup() -> str:
    return (
        ...
        "// which component libraries (Mantine's useMediaQuery / color-scheme\n"
        "// hooks, and others) call on render -- without these stubs every such\n"
        "// component throws 'window.matchMedia is not a function' and EVERY\n"
        "// test fails at render. Harmless for libraries that don't use them.\n"
...
# #8: light/dark neutral palettes for the deterministic theme + product.css
# renderers. The design_tokens.color_scheme picks the palette so a dark design
# actually flows into the generated theme (shadcn and Mantine products share
# these deterministic foundation files).
_SCHEME_PALETTES: dict[str, dict[str, str]] = {
    "light": {
        "text": "#111827",
        "muted": "#6b7280",
        "surface": "#ffffff",
...
def _resolve_scheme_tokens(
    design_constraints: dict[str, Any],
) -> tuple[dict[str, Any], str, dict[str, str], int]:
    """(tokens, scheme, palette, spacing_unit) for the deterministic
    theme.ts / product.css renderers. Unknown values degrade to the light
    defaults -- never crash the build over a malformed DESIGN.json."""
    tokens = design_constraints.get("design_tokens", {}) or {}
    scheme = "dark" if str(tokens.get("color_scheme", "")).lower() == "dark" else "light"
    palette = _SCHEME_PALETTES[scheme]
    try:
        spacing_unit = int(tokens.get("spacing_unit") or 8)
    except (TypeError, ValueError):
        spacing_unit = 8
    if spacing_unit not in (4, 8):
        spacing_unit = 8
    return tokens, scheme, palette, spacing_unit


def _render_theme(design_constraints: dict[str, Any]) -> str:
    tokens, scheme, palette, unit = _resolve_scheme_tokens(design_constraints)
    ...
    spacing_ts = ", ".join(f"{k}: {json.dumps(v)}" for k, v in spacing.items())
    return f"""\
// Generated by the SignalOS local build agent from the approved packet.
export type Theme = {{
  colorScheme: 'light' | 'dark';
  colors: {{ primary: string; text: string; muted: string; surface: string; background: string; border: string }};
  radius: string;
  fontFamily: string;
...
export const theme: Theme = {{
  ...
  colors: {{
    ...
    text: {json.dumps(palette["text"])},
    muted: {json.dumps(palette["muted"])},
    surface: {json.dumps(palette["surface"])},
    background: {json.dumps(palette["background"])},
    border: {json.dumps(palette["border"])},
  }},
...
def _render_product_css(design_constraints: dict[str, Any] | None = None) -> str:
    # #8: the stylesheet is derived from the approved design tokens (color
    # scheme, primary color, border radius, spacing unit, font) instead of a
    # hardcoded light theme, so a dark or dense design flows into the
    # generated product for every UI library.
    tokens, scheme, palette, unit = _resolve_scheme_tokens(design_constraints or {})
    primary = tokens.get("primary_color") or "#2563eb"
    radius = tokens.get("border_radius") or "8px"
    font = tokens.get("font_family") or "Inter, system-ui, sans-serif"
    color_scheme_css = "dark" if scheme == "dark" else "light"
    return f"""\
:root {{
  color-scheme: {color_scheme_css};
  color: {palette["text"]};
