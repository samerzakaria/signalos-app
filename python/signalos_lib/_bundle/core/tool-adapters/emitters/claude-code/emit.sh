#!/usr/bin/env bash
# Claude Code tool-adapter emitter
# Reads canonical JSON registries and rendered preamble,
# writes Claude Code-native config files (.claude/commands/*.md and CLAUDE.md)
...
main() {
  ...
  # Write preamble to CLAUDE.md
  cp "$preamble" "$output_dir/CLAUDE.md"
  ...
  while IFS= read -r name; do
    ...
    local output_file="$output_dir/.claude/commands/${name}.md"
  ...
  echo "Claude Code emitter: wrote $count commands and preamble to $output_dir"
}
