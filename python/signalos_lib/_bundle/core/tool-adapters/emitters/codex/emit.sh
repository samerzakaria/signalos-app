#!/usr/bin/env bash
# Codex tool-adapter emitter
# Reads canonical JSON registries and rendered preamble,
# writes Codex-native config files (codex.md and AGENTS.md with command registry table)
...
main() {
  ...
  # Write preamble to codex.md
  cp "$preamble" "$output_dir/codex.md"

  # Start AGENTS.md with table header
  local agents_file="$output_dir/AGENTS.md"
  {
    echo "# Command Registry"
    echo ""
    echo "| Name | Phase | Description | Source Path |"
    echo "|------|-------|-------------|-------------|"
  } > "$agents_file"

  # Process each command from JSON and add to table
  local count=0
  while IFS= read -r name; do
    ...
    phase=$(jq -r --arg name "$name" '.[] | select(.name == $name) | .phase // "unknown"' "$commands_json")

    # Escape pipe characters in description for markdown table
    desc="${desc//|/\\|}"
    source_path="${source_path//|/\\|}"

    echo "| $name | $phase | $desc | $source_path |" >> "$agents_file"
    count=$((count + 1))
  done < <(jq -r '.[].name' "$commands_json")

  write_signalos_guidance_file "$output_dir" "$obligations_json" "$guidance_catalog_json" "$stack"

  echo "Codex emitter: wrote preamble and $count commands to registry in $output_dir"
}
