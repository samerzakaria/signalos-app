...
- **TDD in VERTICAL SLICES — one behaviour at a time, each slice driven to GREEN before the next.** For each behaviour / acceptance criterion, in one continuous slice: (1) write ONE failing test for it (red), (2) write just enough real implementation to make that one test pass — *including creating the module the test imports* — (3) run `npm test` / `npm run build` and confirm that test is now GREEN, (4) refactor if useful, (5) only THEN start the next behaviour. This is how TDD works for an agent: **one test → make it pass → next.**
- **Do NOT batch all the tests and implement later.** Writing many failing tests and stopping leaves tests without code (`tsc` fails `TS2307: Cannot find module './X'`) and never reaches green — it is the single most common agent-TDD failure and the gate refuses it. Every test you write in a slice is made green in that same slice before you move on.
- **Never stop at red.** Red is a transient state *inside* a slice. Your turn is not complete until every test passes, every import resolves (no `TS2307`), `npm run build` passes, and `npm test` passes.
