...
| Existing repo adoption | In progress | Agent 7 added `init --keep-existing` adoption scanner output: `.signalos/adoption/surface-inventory.json`, `.signalos/adoption/unknowns.json`, `.signalos/adoption/onboarding-draft.md`, `.signalos/adoption/next-steps.md`, and `.signalos/sources/initial-intent.json`; `python -m unittest python.test_adoption_init -v` and `python -m unittest python.test_onboarding -v` pass | Adoption report UI remains downstream |
| Layer 1 factory inputs | In progress | Agent 8 added intent/PRD source capture on the existing `signalos intent` path: prompt sources persist to `.signalos/sources/initial-intent.json`, PRD/spec/document files are copied and fingerprinted under `.signalos/sources/`; `python -m unittest python.test_intent_sources python.test_validate_layer1 -v` and `python -m unittest discover -s python -p "test_intent*.py" -v` pass | Unified factory/new-project/adoption flows still need to call this source-ingestion path |
...
| Layer 2 gate flow | In progress | Agent 10 added shared G0-G5 gate timeline UI, normalized `current` gate status handling, dashboard/sidebar gate rendering, and `wave:begin` inspection publishing into existing gate signals; `npm test` and `npm run build` pass | Reject/request-changes verdict actions remain disabled until backend verdict support is exposed |
