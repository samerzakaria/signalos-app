...
This file documents how to produce a v3.2.3-internal.1 build for **internal testing**
...
5. Writes `distribution/internal/attestation-<short-commit>.json` —
   ...
   ```json
   {
     ...
     "artifacts": [
       {
         "path": "src-tauri/target/release/bundle/nsis/Foundry_3.2.3-internal.1_x64-setup.exe",
         "size": 22118400,
         "sha256": "abc123..."
       }
     ],
...
Outputs by platform:
- macOS: `src-tauri/target/release/bundle/dmg/Foundry_3.2.3-internal.1_*.dmg`
- Linux: `src-tauri/target/release/bundle/{deb,appimage}/Foundry_3.2.3-internal.1_*.{deb,AppImage}`
...
> Hi — this is an **internal testing build** of Foundry v3.2.3-internal.1.
...
>
> **On Windows:** double-click `Foundry_3.2.3-internal.1_x64-setup.exe`. SmartScreen
...
>
> **On Linux:** `chmod +x Foundry_3.2.3-internal.1_amd64.AppImage && ./Foundry_…AppImage`,
> or install the .deb with `sudo dpkg -i …`.
>
> If you want to verify the build wasn't tampered with in transit:
>
> ```bash
> sha256sum Foundry_3.2.3-internal.1_x64-setup.exe
...
```bash
# 1. Tester downloads Foundry_3.2.3-internal.1_x64-setup.exe + attestation-5284391.json
