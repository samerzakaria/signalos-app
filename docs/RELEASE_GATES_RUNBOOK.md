...
This file lists the three release gates that the v3.2.3-internal.1 push **cannot** finish without credentials, hardware, or accounts that live with you, plus the exact commands to run when you do them.
...
The current v3.2.3-internal.1 ship plan is the **internal testing** path. R1–R3 below remain blocked on your credentials. Run `scripts/build-internal.ps1` (or `.sh`) to produce the internal artifacts.
...
2. For each installer, run `minisign -S` with the project private key:
   ...
   minisign -S -s "$TAURI_PRIVATE_KEY_PATH" \
     -m src-tauri/target/release/bundle/nsis/Foundry_3.2.3-internal.1_x64-setup.exe
   ```
...
3. Update both `beta.json` and `latest.json` so each platform stanza has:
   ...
   ```json
   "windows-x86_64": {
     "url": "https://github.com/samerzakaria/signalos-app/releases/download/v3.2.3-internal.1/Foundry_3.2.3-internal.1_x64-setup.exe",
...
2. Copy the signed installer onto the VM:
   ...
   ```
   Foundry_3.2.3-internal.1_x64-setup.exe   (or the .msi)
   ```
