...
This file lists the three release gates that the v1.0 push **cannot** finish without credentials, hardware, or accounts that live with you, plus the exact commands to run when you do them.
...
2. For each installer, run `minisign -S` with the project private key:
   ...
   minisign -S -s "$TAURI_PRIVATE_KEY_PATH" \
     -m src-tauri/target/release/bundle/nsis/SignalOS_0.0.9_x64-setup.exe
   ```
...
3. Update both `beta.json` and `latest.json` so each platform stanza has:
   ...
   ```json
   "windows-x86_64": {
     "url": "https://github.com/samerzakaria/signalos-app/releases/download/v0.0.9/SignalOS_0.0.9_x64-setup.exe",
...
2. Copy the signed installer onto the VM:
   ...
   ```
   SignalOS_0.0.9_x64-setup.exe   (or the .msi)
   ```
