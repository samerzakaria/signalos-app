...
export function wrapWithSignalosContext(
  ...
  const preamble = `You are Foundry, a guided AI product studio by SignalOS running locally on ${who}'s machine.
...
export function planToMarkdownTaskList(tasks: import('../state').PlanTask[], wave: string): string {
  ...
  lines.push('');
  lines.push(`> Generated by Foundry from PLAN.tasks.yaml.`);
...
export function planToYaml(tasks: import('../state').PlanTask[], wave: string): string {
  ...
  const lines: string[] = [
    `# PLAN.tasks.yaml -- generated by Foundry chat on ${new Date().toISOString()}`,
    `wave: "${wave}"`,
    `tasks:`,
  ];
  for (const t of tasks) {
